...
1. **Avoid duplicate origin parsing at startup**
   - Parse `ALLOWED_ORIGINS` once during app initialization and reuse it for both FastAPI middleware and Socket.IO server setup.

2. **Reduce prompt assembly overhead in memory augmentation**
   - Build memory bullets via list comprehension + `"\n".join(...)` instead of repeated string concatenation in loops.

3. **Promote consistent exception observability**
   - Use `logger.exception(...)` for tool failures and external integration calls to preserve stack traces and reduce triage time.
