...
def test_socketio_server_uses_configured_allowed_origins():
    ...
    allowed_origins_from_config = False
    for node in ast.walk(tree):
        if isinstance(node, ast.Assign) and any(isinstance(t, ast.Name) and t.id == "allowed_origins" for t in node.targets):
            if isinstance(node.value, ast.Call) and isinstance(node.value.func, ast.Name):
                allowed_origins_from_config = node.value.func.id == "get_allowed_origins"

    assert allowed_origins_from_config, "Expected allowed_origins to be sourced from get_allowed_origins()"
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Assign):
            ...
            assert isinstance(cors_kw.value, ast.Name), "Expected shared allowed_origins variable"
            assert cors_kw.value.id == "allowed_origins"
            return
