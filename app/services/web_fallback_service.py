...
class WebFallbackService:
    ...
    def _get_search_provider(self) -> SerpAPIProvider:
        # Enforce SerpAPI
    ...
    async def build_fallback_answer(self, query: str, original_answer: str) -> str:
        ...
        if web_results is None:
            try:
                ...
                if "credentials are missing" in str(exc) or "key is missing" in str(exc):
                     return f"{original_answer}\n\n[SYSTEM]: Jag försökte söka på nätet för att verifiera detta, men Sök-konfigurationen saknas (API-nycklar)."
                web_results = []
